...
public interface FineRecordRepository extends JpaRepository<FineRecord, Long> {
    ...
    @Query("select new com.example.lab2.dto.record.FineRecordDTO(fine.fineID,r.time,r.reason,u.username,r.status,r.money) from FineRecord r left join Fine fine on fine.uuid=r.uuid left join User u on " +
